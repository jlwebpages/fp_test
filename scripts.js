
function write_header()
{
   document.open();

   var d = document;

   d.writeln('');
   d.writeln('<table id="header_table_1_desktop" style="margin: 0px auto; width: 75%">');
   d.writeln('   <tr>');
   d.writeln('      <td style="text-align: center; font-family: Tahoma; font-size: 30pt; letter-spacing: 4; padding-top: 50px; padding-bottom: 50px">DARLENE LAGUNA ART</td>');
   d.writeln('   </tr>');
   d.writeln('</table>');
   d.writeln('');
   d.writeln('<table id="header_table_1_mobile" style="margin: 0px auto; width: 75%">');
   d.writeln('   <tr>');
   d.writeln('      <td style="text-align: center; font-family: Tahoma; font-size: 60pt; letter-spacing: 4; padding-top: 50px; padding-bottom: 50px">DARLENE LAGUNA ART</td>');
   d.writeln('   </tr>');
   d.writeln('</table>');
   d.writeln('');
   d.writeln('<table id="header_table_2" style="padding-bottom: 50px; margin: 0px auto; width: 75%">');
   d.writeln('   <tr style="vertical-align: top; text-align: center; font-family: lato; color: darkslategray; font-size: 12pt; letter-spacing: 3; max-width: 50%">');
   d.writeln('      <td>HOME</td>');
   d.writeln('      <td>NEW WORK</td>');
   d.writeln('      <td>FEATURED WORK</td>');
   d.writeln('      <td>PHOTO ART</td>');
   d.writeln('      <td>WORKS ON PAPER</td>');
   d.writeln('      <td>HOW TO PURCHASE</td>');
   d.writeln('      <td>CONTACT</td>');
   d.writeln('   </tr>');
   d.writeln('</table>');
   d.writeln('');

   d.close();

   return true;
}